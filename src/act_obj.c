...
void wear_obj (CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace)
{
    ...
    if (CAN_WEAR (obj, ITEM_WEAR_RELIC))
    {
        if (get_eq_char (ch, WEAR_RELIC_1) != NULL
            && get_eq_char (ch, WEAR_RELIC_2) != NULL
            && !remove_obj (ch, WEAR_RELIC_1, fReplace)
            && !remove_obj (ch, WEAR_RELIC_2, fReplace))
            return;
        ...
        if (get_eq_char (ch, WEAR_RELIC_1) == NULL)
        {
            ...
            equip_char (ch, obj, WEAR_RELIC_1);
            return;
        }

        if (get_eq_char (ch, WEAR_RELIC_2) == NULL)
        {
            act ("$n wears $p.", ch, obj, NULL, TO_ROOM);
            act ("You wear $p.", ch, obj, NULL, TO_CHAR);
            equip_char (ch, obj, WEAR_RELIC_2);
            return;
        }

        bug ("Wear_obj: no free relic slots.", 0);
        send_to_char ("You already wear two relics.\n\r", ch);
        return;
    }
    ...
    if (CAN_WEAR (obj, ITEM_WEAR_CLOTHES))
    {
        if (!remove_obj (ch, WEAR_CLOTHES, fReplace))
            return;
        act ("$n puts on $p.", ch, obj, NULL, TO_ROOM);
        act ("You put on $p.", ch, obj, NULL, TO_CHAR);
        equip_char (ch, obj, WEAR_CLOTHES);
        return;
    }

    if (CAN_WEAR (obj, ITEM_WEAR_ARMOR))
    {
        if (!remove_obj (ch, WEAR_ARMOR, fReplace))
            return;
        act ("$n wears $p.", ch, obj, NULL, TO_ROOM);
        act ("You wear $p.", ch, obj, NULL, TO_CHAR);
        equip_char (ch, obj, WEAR_ARMOR);
        return;
    }
