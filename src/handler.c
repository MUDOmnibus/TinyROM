...
void char_to_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if ((obj = get_eq_char (ch, WEAR_HOLD)) != NULL /* HELDLIGHTS */
...
char *wear_bit_name (int wear_flags)
{
    ...
    if (wear_flags & ITEM_TAKE)
        strcat (buf, " take");
    if (wear_flags & ITEM_WEAR_RELIC)
        strcat (buf, " relic");
    if (wear_flags & ITEM_WEAR_HEAD)
        strcat (buf, " head");
    if (wear_flags & ITEM_WEAR_CLOTHES)
        strcat (buf, " clothes");
    if (wear_flags & ITEM_WEAR_ARMOR)
        strcat (buf, " armor");
